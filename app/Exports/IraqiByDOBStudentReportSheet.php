...
class IraqiByDOBStudentReportSheet implements WithHeadings, WithTitle, ShouldAutoSize, WithEvents
{
    private $data;
    ...
    public function headings(): array
    {
        ...
        $headings = [
            ...
            [
                ...
                'جدول رقم (3) الطلبة العراقيين االموجودين بحسب العمر والجنس للعام الدراسي  ' . $this->academic_year->start_year . '-' . $this->academic_year->end_year,
            ],
            $s,
            $ss
        ...
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                foreach ($rows as $cells) {
                    ...
                    $event->sheet->getDelegate()->setCellValueByColumnAndRow(2, $row_index, $cells[0]->age_group);
                    ...
                    foreach ($cells as $cell) {
                        ...
                        // print sum
                        if ($cell->gender == 0) {
                            $event->sheet->getDelegate()->setCellValueByColumnAndRow($column_index + 1, $row_index, $cell->age_count);
                            $sum += $cell->age_count;
                            $event->sheet->getDelegate()->setCellValueByColumnAndRow($column_index + 3, $row_index, $sum);
                            $total_male += $cell->age_count;
                        } elseif ($cell->gender == 1) {
                            $event->sheet->getDelegate()->setCellValueByColumnAndRow($column_index + 2, $row_index, $cell->age_count);
                            $sum += $cell->age_count;
                            $event->sheet->getDelegate()->setCellValueByColumnAndRow($column_index + 3, $row_index, $sum);
                            $total_female += $cell->age_count;
