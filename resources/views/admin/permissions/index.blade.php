...
@section('content')
    <div class="app-title">
        <div>
            <h1><i class="fa fa-wpforms"></i> ادارة معلومات الصلاحيات </h1>
            <p>يمكنك من خلال هذه الواجه أدارة بيانات الصلاحية.</p>
        </div>
    </div>
            ...
            <div class="card shadow-lg">
                ...
                <div class="card-body">
                    <form method="post" action="{{route('permissions.store')}}" enctype="multipart/form-data"
                        ...
                        <div class="form-group">
                            <label style="border-bottom: 1px solid black; width: 100%" class="pb-1 font-weight-bold">تستخدم في الحساب من نوع:</label>
                    ...
                    <div class="table-responsive">
                        <table id="sampleTable" class="table table-bordered table-striped table-hover" style="width: 100%">
                            ...
                            <tr>
                                ...
                                <th class="text-nowrap">عنوان الصلاحية</th>
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#create_permission_form').on('submit', function (e) {
                e.preventDefault();

                var form = $("form#create_permission_form");
                ...
                axios.post(url, formData)
                    .then(function (response) {
                        if (response.data.hasOwnProperty("message") === true) {
                            ...
                            $("#create_permission_form :input[type=text],#create_permission_form select").removeClass('is-invalid');
                            $("#create_permission_form div.invalid-feedback").remove();
                            var cont = $('#create_permission_form div.alert');
                            cont.text("");
                    ...
                    .catch(function (error) {
                        ...
                        if (error.response.status === 422) {
                            ...
                            $.each(errors, function (key, val) {
                                $("#create_permission_form select[name=" + key + "]").addClass('is-invalid');
                                $("#create_permission_form select[name=" + key + "]").after('<div class="invalid-feedback">' + val + '</div>');
                                $("#create_permission_form input[name=" + key + "]").addClass('is-invalid');
                                $("#create_permission_form input[name=" + key + "]").after('<div class="invalid-feedback">' + val + '</div>');
                            });
                            if (errors.hasOwnProperty('roles')) {
                                cont = $('#create_permission_form div.alert');
