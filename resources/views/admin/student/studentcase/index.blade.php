...
@include('admin.student.studentcase.select_current_course')
@section('styles')
    <style>
...
@section('content')
    <div class="app-title">
        <div>
            <h1><i class="fa fa-wpforms"></i> معالجة حالات الطلبة </h1>
            <p>تمكنك هذه الصفحة من اضافة امر ما للطالب.</p>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <button id="prv_course_btn" type="button"
                            class="btn btn-dark"><i class="fa fa-plus"></i>اختيار كورس الطالب الحالي
                    </button>
        ...
        <div class="col-md-12">
            <div class="card mt-4">
                <div class="card-header">
                    أختر الطلبة المراد تعديل حالتهم الاكاديمية
                </div>
                <div class="card-body">
                    <form id="tarheel_form" action="{{route('cases.store')}}" method="POST">
                        @csrf
                        ...
                        <div class="form-row mt-4">
                            <div class="form-group col-md-4">
                                <select id="academic_status_id" name="academic_status_id"
                                        class="form-control"
                                >
                                    <option value="">أختر الحالة الجديدة</option>
                            ...
                            <div class="form-group col-md-4">
                                <select id="academic_year_id_status" name="academic_year_id_status"
                                        class="form-control"
                                        disabled
                                >
                                    <option value="">سنة العودة الى الدراسة</option>
                                    @foreach($academic_years as $academic_year)
                                        <option value="{{$academic_year->id}}">{{$academic_year->start_year}}-{{$academic_year->end_year}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <button class="btn btn-primary" type="submit" id="send_tarheel_btn"><i
                                            class="fa fa-fw fa-lg fa-check-circle"></i>بدء عملية معالجة حالة الطالب
                                </button>
                            </div>
                        </div>
                    </form>
                ...
                students_table = $('#student_table').DataTable({
                    ...
                    ajax: {
                        ...
                        data: function (d) {
                            ...
                            d.academic_status_id = $('#tarheel_form select[name=academic_status_id]').val();
                        }
                    },
                    columns: [
                        {
                    ...
                    .catch(function (error) {
                        ...
                        if (error.response.status === 422) {
                            ...
                            $.each(errors, function (key, val) {
                                ...
                                $("#tarheel_form textarea[name=" + key + "]").addClass('is-invalid');
                                $("#tarheel_form textarea[name=" + key + "]").after('<div class="invalid-feedback">' + val + '</div>');
                            });
                            if (errors.hasOwnProperty('id')) {
                                ...
                                $('#student_ids_alert').css('display','none');
                            }
                            if (errors.hasOwnProperty('class_id')) {
                                ...
                                $('#prv_c_cont').css('display','none');
                            }
                        }
                    });
            });

            $('#academic_status_id').on('change',function (event) {
                event.preventDefault();
                if($(this).val() === '21'){
                    $('#academic_year_id_status').prop('disabled', false);
                }else{
                    $('#academic_year_id_status').prop('disabled', true);
                }
